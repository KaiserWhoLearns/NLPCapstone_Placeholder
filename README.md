...
`data/` contains the scripts to download the data.

`exp_scripts/` contains the scripts to run experiments.
...
Below is a list of scripts in `exp_scripts`:

* `run_exp.sh` is the general run experiment file. This makes scripts execute in background and print logs into `logs/`. 

* `eval_hf_pretrained.sh` runs evaluation on all the available supervised models (`sup-simcse-bert-base-uncased`, `sup-simcse-bert-large-uncased`, `sup-simcse-roberta-base`, `sup-simcse-roberta-large`) that are made available on Huggingface by the authors.
